...
describe('MCPContentProcessor', () => {
  ...
  describe('analyzeResponse', () => {
    ...
    it('should handle resource content', () => {
      const largeResource = { data: 'x'.repeat(1500), metadata: { type: 'document' } };
      const response = {
        content: [
          { type: 'resource', resource: largeResource }
        ]
      };
  ...
  describe('processResponse', () => {
    ...
    it('should create references for large content', async () => {
      ...
      const processedResponse = result.content as any;
    ...
    it('should handle mixed content correctly', async () => {
      ...
      const processedResponse = result.content as any;
      expect(processedResponse.content[0].type).toBe('content_reference');
      expect(processedResponse.content[1]).toEqual({ type: 'text', text: smallText });
    });
    ...
    it('should include metadata in reference creation', async () => {
      ...
      const result = await processor.processResponse(response, 'wiki-server', 'get-article');
      ...
      const references = Array.from((contentStorage as any).contentStore.entries());
      const [, storedContent] = references[0];
      expect(storedContent.metadata.source).toBe('mcp_tool');
      expect(storedContent.metadata.mcpToolName).toBe('wiki-server::get-article');
  ...
  describe('MIME type detection', () => {
    ...
    it('should detect HTML content', () => {
      const response = {
        content: [
          { type: 'text', text: '<!DOCTYPE html><html><body>content</body></html>' }
        ]
      };
    ...
    it('should default to plain text', () => {
      const response = {
        content: [
          { type: 'text', text: 'Just plain text content' }
        ]
      };
