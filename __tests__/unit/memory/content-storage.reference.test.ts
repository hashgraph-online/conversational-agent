...
describe('ContentStorage - Reference-Based Storage', () => {
  ...
  describe('storeContentIfLarge', () => {
    test('should store large content and return reference', async () => {
      ...
      const reference = await storage.storeContentIfLarge(
        largeContent,
        metadata
      );
  ...
  describe('storeContent', () => {
    test('should store content and return valid reference', async () => {
      ...
      expect(reference.referenceId).toBeDefined();
      expect(
        ReferenceIdGenerator.isValidReferenceId(reference.referenceId)
      ).toBe(true);
      expect(reference.state).toBe('active');
  ...
  describe('performCleanup', () => {
    test.skip('should cleanup expired references', async () => {
      const shortExpiryConfig = {
        ...testConfig,
        maxAgeMs: 50,
        cleanupPolicies: {
          recent: { maxAgeMs: 50, priority: 1 },
          userContent: { maxAgeMs: 50, priority: 2 },
          agentGenerated: { maxAgeMs: 50, priority: 3 },
          default: { maxAgeMs: 50, priority: 4 },
        },
      };
      
      const shortExpiryStorage = new ContentStorage(1000, shortExpiryConfig);
      ...
      try {
        ...
        const reference = await shortExpiryStorage.storeContent(content, metadata);

        await new Promise((resolve) => setTimeout(resolve, 100));

        const result = await shortExpiryStorage.performCleanup();

        expect(result.cleanedUp).toBe(1);
        expect(result.errors).toHaveLength(0);

        const exists = await shortExpiryStorage.hasReference(reference.referenceId);
        expect(exists).toBe(false);
      } finally {
        await shortExpiryStorage.dispose();
      }
    }, 5000);
    ...
    test('should cleanup based on storage limits', async () => {
      ...
      try {
        ...
        await limitedStorage.storeContent(content1, {
          ...metadata,
          sizeBytes: content1.length,
        });
        await limitedStorage.storeContent(content2, {
          ...metadata,
          sizeBytes: content2.length,
        });

        await limitedStorage.storeContent(content3, {
          ...metadata,
          sizeBytes: content3.length,
        });
  ...
  describe('getStats', () => {
    test('should return comprehensive statistics', async () => {
      ...
      const _ref2 = await storage.storeContent(content2, metadata2);
      ...
      expect(stats.averageContentSize).toBe(
        (content1.length + content2.length) / 2
      );
      expect(stats.mostAccessedReferenceId).toBe(ref1.referenceId);
      expect(
        stats.performanceMetrics.averageCreationTimeMs
      ).toBeGreaterThanOrEqual(0);
      expect(
        stats.performanceMetrics.averageResolutionTimeMs
      ).toBeGreaterThanOrEqual(0);
    });
  });
  ...
  describe('content type detection', () => {
    ...
    test('should detect HTML content', async () => {
      const htmlContent =
        '<html><head><title>Test</title></head><body>Content</body></html>'.repeat(
          5
        );
  ...
  describe('content preview generation', () => {
    test('should generate appropriate previews for different content types', async () => {
      const htmlContent =
        '<html><body><h1>Title</h1><p>This is a test paragraph with <strong>bold</strong> text.</p></body></html>'.repeat(
          2
        );
      const reference = await storage.storeContentIfLarge(htmlContent, {
        source: 'mcp_tool',
