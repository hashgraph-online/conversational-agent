...
jest.mock('../../../src/renderer/components/mcp/MCPServerCard', () => ({
  MCPServerCard: ({ server, onToggle, onEdit, onDelete, onTest }: any) => (
    <div data-testid={`server-card-${server.id}`}>
      ...
      <button onClick={() => onEdit(server.id)} data-testid={`edit-${server.id}`}>
        Edit
      </button>
      <button onClick={() => onDelete(server.id)} data-testid={`delete-${server.id}`}>
        Delete
      </button>
      <button onClick={() => onTest(server.id)} data-testid={`test-${server.id}`}>
        Test
      </button>
    </div>
  ),
...
describe('MCPServerList', () => {
  ...
  describe('loading state', () => {
    ...
    it('should not display server cards when loading', () => {
      ...
      expect(screen.queryByTestId('server-card-server-1')).not.toBeInTheDocument()
      expect(screen.getByText('Loading MCP servers...')).toBeInTheDocument()
    })
  })
  ...
  describe('empty state', () => {
    it('should display empty state when no servers are configured', () => {
      ...
      expect(screen.getByText(/Model Context Protocol.*servers extend/)).toBeInTheDocument()
      expect(screen.getByText('Add Your First Server')).toBeInTheDocument()
    })

    it('should call onAdd when "Add Your First Server" button is clicked', () => {
      render(
        <MCPServerList
          servers={[]}
          loading={false}
    ...
    it('should display server icon in empty state', () => {
      render(
        <MCPServerList
          servers={[]}
          loading={false}
