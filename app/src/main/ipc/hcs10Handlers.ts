...
import { HCS10ProfileFormData, HCS10ProfileResponse, HCS10ProfileSchema } from '../../shared/schemas/hcs10';
import { HCS10RegistrationService } from '../services/HCS10RegistrationService';
import { Logger } from '../utils/logger';
import { RegistrationProgressData } from '@hashgraphonline/standards-sdk';

let progressListener: ((progressData: any) => void) | null = null;
...
export function setupHCS10Handlers(): void {
  ...
  if (progressListener) {
    registrationService.removeListener('registrationProgress', progressListener);
  }

  progressListener = (progressData: RegistrationProgressData & { timestamp: string }) => {
   ...
   */
  ipcMain.handle('hcs10:registerProfile', async (
    ...
    try {
      ...
      logger.info('🚀 Starting HCS10 registration with direct SDK functions and progress tracking');

      const initialProgress: RegistrationProgressData & { timestamp: string } = {
        stage: 'preparing',
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to register HCS10 profile' 
      };
    }
  });
   ...
   */
  ipcMain.handle('hcs10:validateProfile', async (
    event: IpcMainInvokeEvent,
    data: unknown
  ): Promise<IPCResponse> => {
    try {
   ...
   */
  ipcMain.handle('hcs10:getProfiles', async (
    event: IpcMainInvokeEvent
  ): Promise<IPCResponse> => {
    try {
   ...
   */
  ipcMain.handle('hcs10:getRegistrationProgress', async (
    event: IpcMainInvokeEvent,
    profileName: string
  ): Promise<IPCResponse> => {
    try {
      const progress = await registrationService.getRegistrationProgress(profileName);
      return {
        success: true,
        data: progress
      };
   ...
   */
  ipcMain.handle('hcs10:isRegistrationInProgress', async (
    event: IpcMainInvokeEvent,
    profileName: string
  ): Promise<IPCResponse> => {
    try {
      const inProgress = await registrationService.isRegistrationInProgress(profileName);
      return {
        success: true,
        data: { inProgress }
      };
   ...
   */
  ipcMain.handle('hcs10:cancelRegistration', async (
    event: IpcMainInvokeEvent
  ): Promise<IPCResponse> => {
    try {
   ...
   */
  ipcMain.handle('hcs10:clearAllStates', async (
    event: IpcMainInvokeEvent
  ): Promise<IPCResponse> => {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to clear registration states'
      };
    }
  });
