...
export class NPMService {
  ...
  private installProgressCallbacks: Map<string, (progress: PluginInstallProgress) => void> = new Map()
  private searchCache: Map<string, { results: PluginSearchResult[]; timestamp: number }> = new Map()
  ...
  async loadPlugins(): Promise<NPMPluginConfig[]> {
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        this.logger.info(`Loaded ${this.pluginConfigs.length} plugin configurations`)
      } else {
        this.pluginConfigs = []
  ...
  async savePlugins(plugins: NPMPluginConfig[]): Promise<void> {
    try {
      ...
      await fs.promises.writeFile(this.configPath, JSON.stringify(plugins, null, 2))
      this.logger.info(`Saved ${plugins.length} plugin configurations`)
    } catch (error) {
      this.logger.error('Failed to save plugin configurations:', error)
      throw error
    }
  }
   ...
   */
  async searchPlugins(query: string, options?: { registry?: string }): Promise<PluginSearchResults> {
    try {
        ...
        npmProcess.on('close', (code) => {
          if (code === 0) {
            try {
              ...
              const pluginResults: PluginSearchResult[] = results.map((pkg: any) => ({
                name: pkg.name,
                version: pkg.version,
  ...
  async installPlugin(
    ...
    try {
      ...
      const existingPlugin = this.pluginConfigs.find(p => p.name === packageName)
      ...
      return new Promise((resolve) => {
        ...
        npmProcess.stdout.on('data', (data) => {
          ...
          if (progressCallback) {
            ...
            } else if (stdout.includes('extracting') || stdout.includes('unpacking')) {
              progressCallback({
                pluginId,
                phase: 'extracting',
                progress: 50,
        ...
        npmProcess.on('close', async (code) => {
          ...
          if (code === 0) {
            try {
              const metadata = await this.readPackageMetadata(installPath, packageName)
              ...
              const updatedConfigs = [...this.pluginConfigs.filter(p => p.id !== pluginId), pluginConfig]
              ...
              resolve({ success: false, error: 'Failed to save plugin configuration' })
            }
          } else {
            if (progressCallback) {
              progressCallback({
  ...
  async uninstallPlugin(pluginId: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const updatedConfigs = this.pluginConfigs.filter(p => p.id !== pluginId)
      await this.savePlugins(updatedConfigs)
  ...
  async checkPluginUpdate(pluginId: string): Promise<PluginUpdateInfo | null> {
    try {
      ...
      const latestVersion = await this.getLatestVersion(plugin.name, plugin.registry)
      if (!latestVersion) return null

      const updateType = this.compareVersions(plugin.installedVersion || plugin.version, latestVersion)
      ...
      this.logger.error('Failed to check plugin update:', error)
      return null
    }
  }
   ...
   */
  private async readPackageMetadata(installPath: string, packageName: string): Promise<PluginMetadata> {
    try {
      ...
      this.logger.error('Failed to read package metadata:', error)
      throw error
    }
  }
   ...
   */
  private extractPermissions(metadata: PluginMetadata): PluginPermissions | undefined {
    const permissions: PluginPermissions = {}

    if (metadata.dependencies && ('fs' in metadata.dependencies || 'fs-extra' in metadata.dependencies)) {
      permissions.filesystem = { read: ['*'], write: [] }
    }

    if (metadata.dependencies && ('axios' in metadata.dependencies || 'node-fetch' in metadata.dependencies)) {
   ...
   */
  private async getLatestVersion(packageName: string, registry?: string): Promise<string | null> {
    try {
   ...
   */
  private compareVersions(current: string, latest: string): 'patch' | 'minor' | 'major' | null {
    try {
      const currentParts = current.split('.').map(Number)
      const latestParts = latest.split('.').map(Number)

      if (latestParts[0] > currentParts[0]) return 'major'
      if (latestParts[0] === currentParts[0] && latestParts[1] > currentParts[1]) return 'minor'
      if (latestParts[0] === currentParts[0] && latestParts[1] === currentParts[1] && latestParts[2] > currentParts[2]) return 'patch'
   ...
   */
  async validatePackageSecurity(packageName: string, version?: string): Promise<{ 
    safe: boolean; 
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        npmProcess.on('close', () => {
          try {
            ...
            const vulnArray = Object.values(vulnerabilities).map((vuln: any) => ({
      ...
      this.logger.error('Package security validation failed:', error)
      return { safe: true }
    }
  }
