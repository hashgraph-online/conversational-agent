...
jest.mock('../../../renderer/components/chat/MessageBubble', () => {
...
describe('MessageList', () => {
  ...
  it('should render empty state when no messages', () => {
    ...
    expect(screen.getByText('Welcome to Conversational Agent')).toBeInTheDocument();
    expect(screen.getByText(/I can help you with Hedera network operations/)).toBeInTheDocument();
  });
  ...
  it('should show example questions in empty state', () => {
    ...
    expect(screen.getByText('• "What\'s my account balance?"')).toBeInTheDocument();
    expect(screen.getByText('• "Transfer 5 HBAR to 0.0.123456"')).toBeInTheDocument();
    expect(screen.getByText('• "Help me create a new account"')).toBeInTheDocument();
    expect(screen.getByText('• "Send a message to HCS topic"')).toBeInTheDocument();
  });
  ...
  it('should not show empty state when messages are present', () => {
    ...
    expect(screen.queryByText('Welcome to Conversational Agent')).not.toBeInTheDocument();
  });
