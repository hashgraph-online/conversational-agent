...
import { FiChevronDown, FiChevronUp, FiInfo, FiRefreshCw } from 'react-icons/fi'
import { ModelInfo, getModelsByProvider, getModelDisplayInfo } from '../../lib/models'
...
const ModelCard: React.FC<ModelCardProps> = ({ model, isSelected, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <Typography variant="caption" color="muted" className="mb-3 leading-relaxed">
        {model.description}
      </Typography>
      ...
      {model.bestFor && model.bestFor.length > 0 && (
        <div className="mb-3">
          <Typography variant="caption" className="font-medium text-gray-700 dark:text-gray-300 mb-1">
            Best For:
          </Typography>
...
export const ModelSelectorWithOpenRouter: React.FC<ModelSelectorProps> = ({
  ...
  const { models: openRouterModels, isLoading, error, refetch } = useOpenRouterModels({
    provider,
    autoLoad: showLiveData
  })
  
  const staticModels = getModelsByProvider(provider)
  const models = showLiveData && openRouterModels.length > 0 ? openRouterModels : staticModels
    ...
    <div className={className}>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <Typography variant="caption" color="muted">
            {showLiveData ? 'Live pricing from OpenRouter' : 'Choose the model that best fits your use case'}
          </Typography>
        </div>
        <div className="flex items-center space-x-2">
          {useOpenRouter && (
      ...
      {isLoading ? (
        ...
        <>
          {recommendedModels.length > 0 && (
            <div className="mb-6">
              <Typography variant="h6" className="mb-3 text-green-700 dark:text-green-300">
                ⭐ Recommended Models
              </Typography>
