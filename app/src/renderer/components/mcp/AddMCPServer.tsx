...
import { FiServer, FiDatabase, FiGithub, FiHardDrive, FiSettings, FiInfo } from 'react-icons/fi'
import {
  Dialog,
  DialogContent,
  DialogHeader,
 ...
 */
const FieldRequirement: React.FC<{ requirement: string }> = ({ requirement }) => {
...
export const AddMCPServer: React.FC<AddMCPServerPropsExtended> = ({
  ...
  const [fieldRequirements, setFieldRequirements] = useState<Record<string, string>>({})
  ...
  useEffect(() => {
    ...
    const requirements = MCPServerFormValidator.getAllFieldRequirements(watchedType)
  ...
  useEffect(() => {
    if (editingServer) {
      ...
      if (templateData.rootPath === '$HOME') {
        templateData.rootPath = process.env.HOME || process.env.USERPROFILE || ''
      }
  ...
  const validateField = (field: string, value: any) => {
    const validators = MCPServerFormValidator.getValidationHelpers(selectedType)
    const defaultValidators = MCPServerFormValidator.getValidationHelpers('filesystem')
  ...
  const handleFormSubmit = async (data: FormData) => {
    ...
    try {
      const allErrors = MCPServerFormValidator.validateAllFields(data.type, data)
      ...
      switch (data.type) {
        case 'filesystem':
          ...
          if (data.allowedPaths) {
            config.allowedPaths = data.allowedPaths.split(',').map(p => p.trim())
          }
          if (data.excludePaths) {
            config.excludePaths = data.excludePaths.split(',').map(p => p.trim())
          }
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <Input
                ...
                className={cn(
                  validationErrors.rootPath && 'border-red-500 focus:border-red-500'
                )}
              />
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <Input
                ...
                className={cn(
                  validationErrors.token && 'border-red-500 focus:border-red-500'
                )}
              />
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                {...register('repo', { required: 'Repository name is required' })}
              />
              {errors.repo && (
                <Typography variant="caption" className="text-red-600 dark:text-red-400">
                  {errors.repo.message}
            ...
            <div className="space-y-1">
              ...
              <div className="mt-1">
                <Typography variant="caption" color="muted" className="block">
                  Enter an npm package name (e.g., @your-org/mcp-server) to run with npx, or a full path to a local executable
                </Typography>
              </div>
            </div>
            <div className="space-y-1">
        ...
        {template?.requirements && template.requirements.length > 0 && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                <div className="mb-2">
                  <Typography variant="body1" className="font-medium text-blue-900 dark:text-blue-100">
                    Setup Requirements
                  </Typography>
                </div>
