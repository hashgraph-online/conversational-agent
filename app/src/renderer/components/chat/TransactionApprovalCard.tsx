...
import { FiClock, FiLoader, FiCheckCircle, FiExternalLink, FiCopy } from 'react-icons/fi';
import { cn } from '../../lib/utils';
import { useNotificationStore } from '../../stores/notificationStore';
import { HederaMirrorNode, Logger, TransactionParser } from '@hashgraphonline/standards-sdk';
 ...
 */
export const TransactionApprovalCard: React.FC<TransactionApprovalCardProps> = ({
  scheduleId,
  description,
  className
}) => {
      ...
      <div className="flex flex-col space-y-4">
        <div className="flex items-start">
          ...
          <div className="ml-4 flex-1">
            <Typography variant="h6" className="font-semibold text-purple-800 dark:text-purple-200">
              {isExecuted ? 'Transaction Executed' : isApproved ? 'Transaction Approved' : 'Transaction Requires Approval'}
            </Typography>
          ...
          <div className="space-y-3">
            ...
            {scheduleInfo?.expirationTime && !isExecuted && (
              <div className="bg-white/50 dark:bg-gray-900/30 rounded-lg p-3">
                ...
                <Typography variant="body2">{formatTime(scheduleInfo.expirationTime)}</Typography>
              </div>
            )}
            ...
            {scheduleInfo?.parsedTransaction ? (
              <TransactionDetails
                ...
                humanReadableType={scheduleInfo.parsedTransaction.humanReadableType}
                transfers={scheduleInfo.parsedTransaction.transfers?.map(t => ({
                ...
                hideHeader={!!description && (
                  description.includes(scheduleInfo.parsedTransaction.humanReadableType) ||
              ...
              <div className="bg-white/50 dark:bg-gray-900/30 rounded-lg p-3">
                ...
                <Typography variant="body2" className="text-xs text-gray-600 dark:text-gray-400">
                  Unable to parse transaction details. Raw transaction body available.
                </Typography>
              </div>
            )}
          </div>
