...
export interface ElectronAPI {
  ...
  loadMCPServers: () => Promise<{ success: boolean; data?: any; error?: string }>
  saveMCPServers: (servers: any) => Promise<{ success: boolean; error?: string }>
  testMCPConnection: (server: any) => Promise<{ success: boolean; data?: any; error?: string }>
  connectMCPServer: (serverId: string) => Promise<{ success: boolean; error?: string }>
  disconnectMCPServer: (serverId: string) => Promise<{ success: boolean; error?: string }>
  getMCPServerTools: (serverId: string) => Promise<{ success: boolean; data?: any; error?: string }>
  
  searchMCPRegistry: (options?: any) => Promise<{ success: boolean; data?: any; error?: string }>
  ...
  searchPlugins: (query: string, options?: any) => Promise<{ success: boolean; data?: any; error?: string }>
  installPlugin: (packageName: string, options?: PluginInstallOptions) => Promise<{ success: boolean; data?: any; error?: string }>
  uninstallPlugin: (pluginId: string) => Promise<{ success: boolean; error?: string }>
  updatePlugin: (pluginId: string, version?: string) => Promise<{ success: boolean; error?: string }>
  ...
  getPluginPermissions: (pluginId: string) => Promise<{ success: boolean; data?: PluginPermissions; error?: string }>
  grantPluginPermissions: (pluginId: string, permissions: PluginPermissions) => Promise<{ success: boolean; error?: string }>
  getInstalledPlugins: () => Promise<{ success: boolean; data?: any[]; error?: string }>
  checkPluginUpdates: () => Promise<{ success: boolean; data?: any[]; error?: string }>
  validatePluginConfig: (pluginId: string, config: Record<string, any>) => Promise<{ success: boolean; data?: { valid: boolean; errors?: string[] }; error?: string }>
  validatePluginSecurity: (packageName: string) => Promise<{ success: boolean; data?: { safe: boolean; issues?: string[] }; error?: string }>
  ...
  onPluginUpdateProgress: (callback: (progress: PluginInstallProgress) => void) => void
}
