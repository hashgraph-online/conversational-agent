...
const electronBridge = {
  ...
  testHederaConnection: (credentials: Record<string, unknown>) => ipcRenderer.invoke('connection:test-hedera', credentials),
  testOpenAIConnection: (credentials: Record<string, unknown>) => ipcRenderer.invoke('connection:test-openai', credentials),
  testAnthropicConnection: (credentials: Record<string, unknown>) => ipcRenderer.invoke('connection:test-anthropic', credentials),
  ...
  initializeAgent: (config: Record<string, unknown>) => ipcRenderer.invoke('agent:initialize', config),
  preloadAgent: (config: Record<string, unknown>) => ipcRenderer.invoke('agent:preload', config),
  disconnectAgent: () => ipcRenderer.invoke('agent:disconnect'),
  sendAgentMessage: (data: Record<string, unknown>) => ipcRenderer.invoke('agent:send-message', data),
  
  loadMCPServers: () => ipcRenderer.invoke('mcp:loadServers'),
  saveMCPServers: (servers: Record<string, unknown>[]) => ipcRenderer.invoke('mcp:saveServers', servers),
  testMCPConnection: (server: Record<string, unknown>) => ipcRenderer.invoke('mcp:testConnection', server),
  ...
  searchMCPRegistry: (options: Record<string, unknown>) => ipcRenderer.invoke('mcp:searchRegistry', options),
  ...
  installPlugin: (packageName: string, options?: Record<string, unknown>) => ipcRenderer.invoke('plugin:install', { packageName, options }),
  ...
  configurePlugin: (pluginId: string, config: Record<string, unknown>) => ipcRenderer.invoke('plugin:configure', { pluginId, config }),
  grantPluginPermissions: (pluginId: string, permissions: Record<string, unknown>) => ipcRenderer.invoke('plugin:grantPermissions', { pluginId, permissions }),
  revokePluginPermissions: (pluginId: string, permissions: Record<string, unknown>) => ipcRenderer.invoke('plugin:revokePermissions', { pluginId, permissions }),
  getInstalledPlugins: () => ipcRenderer.invoke('plugin:getInstalled'),
  checkPluginUpdates: () => ipcRenderer.invoke('plugin:checkUpdates'),
  validatePluginConfig: (pluginId: string, config: Record<string, unknown>) => ipcRenderer.invoke('plugin:validateConfig', { pluginId, config }),
  ...
  executeTransactionBytes: (transactionBytes: string, entityContext?: { name?: string, description?: string }) => ipcRenderer.invoke('execute-transaction-bytes', transactionBytes, entityContext),
